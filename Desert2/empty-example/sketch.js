...
function mountain_range(scaleX, tx, ty, background_color, draw_tree, layer_scale, density)
{
  push();
    ...
    // {  for (i = -width/2; i < width; i+= noise(i)*(density))
    //   {
    ...
    //         raise *= 30 ; 
    //         fill(random(0.6, 0.8), random(0.5, 0.8), 0.2)
...
function setup()
{
    let img = createCanvas(3840, 2160);
     // = height/2
    ...
    // fill(random(0.5, 1), 0.5, 0.2)// tree color


    // mountain_range(120, 250, 80, 0.8, 0, 1)

    mountain_range(100, 1350, 150, 0.6, 1, 0.8, 8)

    // mountain_range(100, 350, 275, 0.3, 1, 1, 6) //scaleX, tx, ty, color, alpha, draw_tree

    // mountain_range(60, 350, 550, 0.1, 1 , 1.5, 8)

        save(img);

    // mountain_range(100, 1200, 700, 0.1, 1, 1)
